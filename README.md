...
### 延伸实验：weight_decay 单变量消融（`04_weight_decay.py`）

在 03 极简协议基础上唯一变量为 MLP 的 Adam 加 `weight_decay=1e-4`（SingleLayerNet 不加，作对照锚；其余协议完全一致），实测对照（30% 测试集，取自 `inbox/results_wd.json`）：

| 模型 | MSE | RMSE | MAE | R² |
|---|---|---|---|---|
| MLP（03，无 weight_decay） | 44528.0 | 211.0 | 125.7 | 0.8904 |
| MLP（04，weight_decay=1e-4） | 39484.6 | 198.7 | 113.8 | 0.9028 |

结论：单靠 weight_decay 找回约 1.2 个百分点 R²（0.8904 → 0.9028，RMSE 降 12.3），验证了"正则化组件是旧调参版优势来源之一"的归因，但距旧调参版 0.9236 仍有约 2.1 个百分点——归因于其余组件（归一化层、Dropout）与超参搜索，主线仍保持 03 的极简协议。
...
├── 04_weight_decay.py        # 延伸实验：MLP Adam weight_decay=1e-4 单变量消融
├── requirements.txt
└── README.md
```
