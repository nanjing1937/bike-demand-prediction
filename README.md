...
预期输出：`inbox/figures/` 下 8 张 PNG、`data/processed/` 下 4 个 csv、`inbox/results_summary.json`。03 全流程（含三模型 RandomizedSearchCV 调参）约 3~4 分钟。
...
7. **模型阵容**：LinearRegression / Ridge / Lasso（基线+正则对比）+ RandomForest / XGBoost / LightGBM（非线性主力）

## ④ 测试与验证方式

- **复现**：全流程 random_state=42（数据划分、KFold、RandomizedSearchCV、模型种子），重跑三个脚本应得到与 `inbox/results_summary.json` 完全一致的数字
- **核对**：`inbox/results_summary.json` 含六模型 MSE/RMSE/MAE/R² + 最优超参 + 最优模型残差统计
- **合理性范围**：线性基线 R²≈0.57，树模型 R² 0.93~0.95，最优 RMSE≈144（约为目标均值 704 的 20%）；若结果偏差大，先查数据清洗是否生效（行数应为 8700）
